...
def plot_tracking_matplotlib(result: MinuteTrackingResult, index_name: str = ""):
    ...
    ax1.plot(x, index_pct.values, linewidth=1.3, color="#1f77b4", label="Real index 1m close/preClose")
...
def save_tracking_outputs(
    ...
    *,
    ...
    curve_path = output_dir / f"minute_tracking_v5_{label}.csv"
    _curve_frame(result).to_csv(curve_path, index=False, encoding="utf-8-sig")

    stats_path = output_dir / f"minute_tracking_stats_v5_{label}.csv"
    ...
    html_path = output_dir / f"minute_tracking_v5_{label}.html"
