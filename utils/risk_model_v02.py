...
def _extract_minute_price_series(frame: pd.DataFrame, price_col: str) -> pd.Series:
    ...
    in_session = series.index.map(
        lambda value: 930 <= value <= 1130 or 1300 <= value <= 1500
    )
    ...
    *,
    ...
    for date_key, tick_dict in sorted(tick_dict_by_date.items()):
        ...
        if log_returns.empty:
            continue
        log_returns.index = [
            f"{date_key}_{int(minute):04d}" for minute in log_returns.index
        ]
...
def _day_decay_weights(
    ...
    day_age = {
        day: len(unique_days) - 1 - position
        for position, day in enumerate(unique_days)
    }
    raw = np.array(
        [0.5 ** (day_age[day] / float(half_life_days)) for day in day_keys],
        dtype=float,
    )
