...
def save_tracking_outputs(
    ...
    *,
    ...
    curve_path = output_dir / f"minute_tracking_v9_{label}.csv"
    _curve_frame(result).to_csv(curve_path, index=False, encoding="utf-8-sig")

    stats_path = output_dir / f"minute_tracking_stats_v9_{label}.csv"
    _stats_frame(result, daily_results).to_csv(stats_path, index=False, encoding="utf-8-sig")

    quantity_adjustments_path = output_dir / f"minute_tracking_quantity_adjustments_v9_{label}.csv"
    dividend_records_path = output_dir / f"minute_tracking_dividends_v9_{label}.csv"
    dividend_summary_path = output_dir / f"minute_tracking_dividend_summary_v9_{label}.csv"
    ...
    html_path = output_dir / f"minute_tracking_v9_{label}.html"
