...
def _prepare_real_index_minute_series(
    ...
    *,
    ...
    valid_amount_modes = {"basket_base_scaled", "contract_notional"}
    ...
    if real_index_amount_mode == "basket_base_scaled":
        ...
        if index_units is None or contract_multiplier is None:
            raise ValueError("index_units and contract_multiplier are required for contract_notional mode.")
        if not np.isfinite(index_units) or index_units <= 0:
            raise ValueError("index_units must be positive for contract_notional mode.")
        if not np.isfinite(contract_multiplier) or contract_multiplier <= 0:
            raise ValueError("contract_multiplier must be positive for contract_notional mode.")
        base_price = float(previous_index_close) if previous_index_close is not None else np.nan
        amount_values = index_price * float(index_units) * float(contract_multiplier)
...
def combine_minute_tracking_results(results_by_date: dict[str, MinuteTrackingResult]) -> MinuteTrackingResult:
    ...
    for date_key, result in ordered_items:
        if abs(result.basket_base_amount - basket_base_amount) > 1e-6:
            raise ValueError(f"basket_base_amount differs on {date_key}.")
...
def _stats_frame(result: MinuteTrackingResult, daily_results: dict[str, MinuteTrackingResult] | None = None) -> pd.DataFrame:
    ...
    for col in ("daily_stats", "real_index_preclose_by_date", "fitting_dates"):
...
def save_tracking_outputs(
    ...
    *,
    ...
    curve_path = output_dir / f"minute_tracking_v7_{label}.csv"
    _curve_frame(result).to_csv(curve_path, index=False, encoding="utf-8-sig")

    stats_path = output_dir / f"minute_tracking_stats_v7_{label}.csv"
    _stats_frame(result, daily_results).to_csv(stats_path, index=False, encoding="utf-8-sig")

    quantity_adjustments_path = output_dir / f"minute_tracking_quantity_adjustments_v7_{label}.csv"
    dividend_records_path = output_dir / f"minute_tracking_dividends_v7_{label}.csv"
    dividend_summary_path = output_dir / f"minute_tracking_dividend_summary_v7_{label}.csv"
    ...
    html_path = output_dir / f"minute_tracking_v7_{label}.html"
